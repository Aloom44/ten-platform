...
export const Profile: React.FC = () => {
  return (
    <div className="pb-24 pt-8">
      <div className="flex flex-col items-center px-6 mb-8">
        <div className="relative mb-4 group cursor-pointer">
          <div className="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-sky-400 to-blue-500 shadow-xl">
             <img src={MOCK_PROFILE.avatar} alt="Avatar" className="w-full h-full rounded-full border-4 border-white object-cover" />
          </div>
        ...
        <h2 className="text-2xl font-black text-slate-800 mb-1">{MOCK_PROFILE.name}</h2>
        <span className="bg-sky-100 text-sky-600 px-3 py-1 rounded-full text-xs font-bold">المستوى {MOCK_PROFILE.level}</span>
      </div>
      ...
      <div className="px-6 mb-8">
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-sm font-bold text-slate-400">{MOCK_PROFILE.progress}%</span>
          </div>
      ...
      <div className="px-6">
        <h3 className="font-bold text-slate-800 mb-4 px-2">أوسمتي ({MOCK_PROFILE.badges.length})</h3>
        <div className="grid grid-cols-4 gap-4">
          {MOCK_PROFILE.badges.map((badge, idx) => (
